...
;;; (setq rfc-archive-alist (list rfc-url-save-directory "http://www.ietf.org/rfc/"))
...
(defun rfc-insert-contents-file (archive filename)
  (condition-case nil
      (insert-file-contents (concat archive "/" filename ".gz"))
    (error nil)))
...
(defun rfc-url-save (filename)
  (let ((full-filename (concat rfc-url-save-directory "/" filename)))
    (write-file (concat rfc-url-save-directory "/" filename))
    (shell-command (format "gzip %s" full-filename))
    (set-visited-file-name nil)))

(defun rfc-url-index-update ()
  (interactive)
  (condition-case nil
      (delete-file (concat rfc-url-save-directory "/rfc-index.txt.gz"))
    (error nil))
