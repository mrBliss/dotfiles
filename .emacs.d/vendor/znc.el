...
;; Types
(defconst *znc-server-accounts-type*
  '((cons :tag "Account"
  ...
  (let* ((buffer (znc-network-server-buffer network))
         ...
         (pending (and buffer proc
                       (erc-with-all-buffers-of-server proc
                         (lambda () (not (equal (current-buffer)
                                           (erc-server-buffer))))
                         (current-buffer)))))
    (if buffer
...
(defadvice erc-server-reconnect (after znc-erc-rename last nil activate)
  ...
  (let* ((wants-name (and (local-variable-p 'znc-buffer-name
                                            (erc-server-buffer))
                          (buffer-local-value 'znc-buffer-name
                                              (erc-server-buffer))))
...
;; Hooks
(defun znc-kill-channel-hook ()
  "Hook that handles ZNC-specific channel killing behavior"
  (and (local-variable-p 'znc-buffer-name (erc-server-buffer))
       znc-detatch-on-kill
       (znc-detach-channel)))

(add-hook 'erc-kill-channel-hook 'znc-kill-channel-hook)

;;; Traversal
(defun* znc-walk-all-servers (&key (each (lambda (&rest r)
                                           (mapcar 'identity r)))
                                   (pred (lambda (&rest _) t))
                                   (first nil))

  "Walk ever defined server and user pair calling `each' when `pred' is non-nil
...
;; Indent it properly
(put 'with-endpoint 'lisp-indent-function 1)
...
(defun znc-set-name (znc-name &optional buffer)
  "Set the znc-buffer-name buffer local to znc-name in buffer
or (current-buffer)."
...
(defun znc-erc-connect (endpoint)
  (with-endpoint endpoint
    (let* ((buffer (znc-network-buffer-name slug))
           ...
           (erc-args `(:server ,host :port ,port :nick ,user
